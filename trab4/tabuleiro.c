...
void Imprime(LIS_tppLista pLista) {
    int i,j;
    char cor,id;
    LIS_tppLista nova,nova2,nova3;
    LIS_tppLista noLista = NULL;
    pCasa noCasa,noCasa2;
    IrInicioLista(pLista);
    /*
    LIS_CriarLista((LIS_tppLista *)&nova, a, destruirValor);
    LIS_CriarLista((LIS_tppLista *)&nova2, a, destruirValor);
    LIS_InserirNo(nova, (void *)nova2);
    TAB_CriaCasa((Casa **)&noCasa, 'V', 'V');
    LIS_InserirNo(nova2, (void *)noCasa);
    TAB_CriaCasa((Casa **)&noCasa, 'P', 'B');
    LIS_InserirNo(nova2, (void *)noCasa);
    
    LIS_ObterNo(nova, (void **)&nova3);
    LIS_ObterNo(nova2, (void **)&noCasa2);
    PEC_RetornaCor(noCasa2->elemento, &cor);
    PEC_RetornaId(noCasa2->elemento, &id);
    LIS_ObterNo(nova2, (void **)&noCasa2);
    
    
    printf("passou disso (%c,%c)\n",cor,id);
    
    */
    IrInicioLista(pLista);
    for (i=0; i<8; i++) {
        LIS_ObterNo(pLista, (void **)&noLista);
        IrInicioLista(noLista);
        for (j=0; j<8; j++) {
            LIS_ObterNo(noLista, (void **)&noCasa);
            PEC_RetornaCor((Peca *)noCasa->elemento, &cor);
            PEC_RetornaId((Peca *)noCasa->elemento, &id);
            printf("(%c,%c)\n",cor,id);
            LIS_IrProx(noLista);
        }
        LIS_IrProx(pLista);
    ...
    for(i=0;i<8;i++) {
        ...
        for(j=0;j<8;j++) {
            if(i<j) {
                ...
                TAB_CriaCasa((Casa **)&(novaCasa),'C','P');
            }
            ...
            if(corPecaUsada != 'V' && idPecaUsada != 'V') {
                for(k=0;k<qtdMov;k++) {
                    ...
                    if(idPecaUsada != 'P' && idPecaUsada  != 'R') {
                        ...
                        if(k == 0) {
                            if(corPecaUsada == 'B') {
                                retTab = TAB_ObterPeca(tabu, i + 1, j + 1,&corPecaUsada2 ,&idPecaUsada2);
                                printf("retTab: %d\n",retTab);
                                if(retTab == TAB_CondRetOK) {
                                    if(idPecaUsada2 != 'V' && corPecaUsada2 == 'P') {
                                        printf("Entre aqui peao (%d,%d) bla\n",i + 1,j + 1);
                                        PEC_CriaPeca(&pecaUsada2,corPecaUsada2,idPecaUsada2);
                                        LIS_InserirNo(tabu->tab[i + 1][j + 1].ameacantes, (void *)pecaUsada);
                                ...
                                retTab = TAB_ObterPeca(tabu, i + 1, j - 1,&corPecaUsada2 ,&idPecaUsada2);
                                if(retTab == TAB_CondRetOK) {
                                    if(idPecaUsada2 != 'V' && corPecaUsada2 == 'B') {
                                        PEC_CriaPeca(&pecaUsada2,corPecaUsada2,idPecaUsada2);
                                        LIS_InserirNo(tabu->tab[i + 1][j - 1].ameacantes, (void *)pecaUsada);
                                        LIS_InserirNo(tabu->tab[i][j].ameacados, (void *)pecaUsada2);
                                    }
                    ...
                    if(verificaMov == 2) {
                        ...
                        if (verificaCoordenadas(i + xObtido, j + yObtido) == 0) {
                            continue;
                        }
                        ...
                        LIS_InserirNo(tabu->tab[i][j].ameacados, (void *)pecaUsada2);
                        printf("peca2: %d\n",pecaUsada2);
                        LIS_ObterNo(tabu->tab[i][j].ameacados, (void **)&pecaUsada3);
...
TAB_tpCondRet TAB_VerificaCheckMate(ptTabuleiro tabu,int xRei,int yRei,int xAmeacante,int yAmeacante) {
    ...
    rei = (Peca *)tabu->tab[xRei][yRei].elemento;
    printf("(%d,%d)\n",xAmeacante,yAmeacante);
    /*
     if(verificaCoordenadas(xAmeacante, yAmeacante) == 0) {
     return TAB_CondRetCoordenadaNExiste;
     }
     */
    ...
    if(numElemLista>0) {
        ...
        return TAB_CondRetComeuParaSalvar;
    }
